...
def get_stat_dict(y, probs):
	...
	if num_class == 2:	
		...
		precision, recall, _ = precision_recall_curve(y, prob_mean[:, 1])
		stat_dict["AUPR"] = auc(recall, precision)
		stat_dict["Accuracy"] = accuracy_score(y, lab)
	tn, fp, fn, tp = confusion_matrix(y, lab).ravel()
	precision = tp / (tp + fp)
	recall = tp / (tp + fn)
	Gmean = np.sqrt(precision * recall)
	stat_dict['Gmean'] = Gmean
